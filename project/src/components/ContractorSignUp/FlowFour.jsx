...
export default function CsignUpFour({ onNext, onBack }) {
  ...
  const handleNextClick = () => {
    // Portfolio upload is optional, so we don't require files
    const formData = { uploadedFiles: uploadedFiles.map((f) => f.name) }
        ...
        <div className="mb-8">
          ...
          <div className="relative">
            <div className="w-full h-2 bg-gray-300 rounded-full">
              <div className="w-[100%] h-2 bg-blue-600 rounded-full"></div>
            </div>
          </div>
        </div>
